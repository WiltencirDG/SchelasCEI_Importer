...
async function robot(ceiCredentials, pBroker) {
    ...
    async function navigateToCEI(page) {
        await page.goto('https://ceiapp.b3.com.br/CEI_Responsivo/', { waitUntil: 'networkidle2' });
    }
    ...
    async function navigateToAssets(page) {
        await page.goto('https://ceiapp.b3.com.br/CEI_Responsivo/negociacao-de-ativos.aspx', { timeout: 60000 })
    }
    ...
    async function getAllBrokers(page, pBroker) {
        ...
        let newBrokersArray = []
        if (pBroker) {
            let pBrokers = pBroker.split(',')
            pBrokers.forEach((br) => {
                newBrokersArray.push(brokersArray.filter((brok) => brok.indexOf(br) > -1)[0])
            })
        }
        if (newBrokersArray.length > 0) {
            brokersArray = newBrokersArray
        }
